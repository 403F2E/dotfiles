...
other_monitor=$(hyprctl monitors -j | jq -r '.[] | select(.focused != true) | .name')
...
no-effects() {
    ...
    swww img -o "$other_monitor" "$wallpaper_current" $SWWW_PARAMS &&
    wait $!
...
main() {
    ...
    if [[ -n "$choice" ]]; then
        if [[ "$choice" == "No Effects" ]]; then
            ...
            swww img -o "$other_monitor" "$wallpaper_output" $SWWW_PARAMS &
